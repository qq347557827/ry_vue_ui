import zhCnNames from 'zh-address-parse/app/lib/names.json'
import addressJson from 'china-division/dist/pca-code.json'
 ...
 * @returns {{}|({area: Array, province: Array, phone: string, city: Array, name: string, editor: Array} & {area: (*|string), province: (*|string), city: (*|string), editor: (Array|boolean|string|string)})}
 * @constructor
 */
const AddressParse = (address, options) => {
  ...
  log('分割地址 --->splitAddress', splitAddress)
...
const sortAddress = (splitAddress) => {
  ...
  // 按照关键字的顺序，提取并移除原数组中包含该关键字的项，放入结果数组
 ...
 * @returns {{area: (Array|*|string), province: (Array|*|string), city: (Array|*|string|string), editor: (*|Array)}}
 */

// fragment 是sortAddress foreach 的item
const parseRegionWithRegexp = (fragment, hasParseResult) => {
  log('----- 当前使用正则匹配模式 -----')
  // 从已有的解析结果中提取省、市、区、详细地址信息，若不存在则设置为空数组
  ...
  if (province.length === 0) {
    for (let i = 1; i < fragment.length; i++) {
      ...
      // 在省份字符串中进行匹配
      const matchProvince = provinceString.match(regexProvince)// provinceStrings是省份信息数组的json字符串
      // 如果匹配到省份信息
      if (matchProvince) {
        const provinceObj = JSON.parse(matchProvince[0])
        // 由短到长查询，如果匹配到了，就被上一次找到的短的清空，从新添加新的找到的
        // 比如上一次找到了湖南，添加到了province，这一次找到了湖南省，就把province清空，把湖南省加入
  ...
  if (city.length === 0) {
    ...
    if (city[0]) {
      console.log('匹配城市city[0]', city[0])
      const { provinceCode, name } = city[0]
      // 判断是否为地区 或者 自治州
      const isZZZ = name.indexOf('自治州') >= 0 || name.indexOf('地区') >= 0
      const index = fragment.indexOf(matchStr) + matchStr.length
      const str = fragment.slice(index, index + 1)
      console.log(str)
      const isSX = str.indexOf('市') >= 0 || str.indexOf('县') >= 0
  ...
  console.log('area匹配城市city[0]', city[0])
  // console.log("area匹配城市city[0].code", city[0].code)
  if (area.length === 0) {
    console.log('area匹配城市city[0]', city[0])
    for (let i = 1; i < fragment.length; i++) {
      const str = fragment.substring(0, i + 1)
    ...
    // console.log("city[0].code.length >= 6", city[0].code.length >= 6)
    if (area[0]) {
 ...
 * @param fragment
 * @param hasParseResult
 * @returns {{area: Array, province: Array, city: Array, editor: Array}}
 */
      ...
      if (currentProvince) {
        if (currentProvince.code === provinceCode) {
          ...
          for (let i = name.length; i > 1; i--) {
            const temp = name.substring(0, i)
            if (fragment.indexOf(temp) === 0) {
              // 判断name是否包含地区 和 fragment
              // const position = fragment.indexOf(temp);
              // const startPos = position + i
              // const str = fragment.slice(startPos, startPos +1)
              // console.log("// 判断name是否包含地区 和 fragment", str)
              // console.log("ctiyName",name)
              // const flg = name.indexOf("地区") > 1 && str === "市"
              // console.log("name.indexOf(\"地区\") === 0 && str === \"市\"", flg)
              // if (!flg)  {
              //   replaceName = temp
              // }
              replaceName = temp
